...
### 需求
读取系统开机时长，并据此计算「从开机到现在的累计电费」。

### 实现
- **开机时长读取**：复用既有的 `sample_dynamic()` → `psutil.boot_time()`（每拍刷新，
  无需新依赖）。`_sys_dyn["boot"]` 已在 v18.33 接通，本版只是在此基础上做费用换算。
- **电费估算**：`开机秒数 ÷ 3600 × 当前插座功耗(kW) × 有效电价`。
  - 有效电价 `blended`：峰谷/阶梯模式下取本次已累计平均单价（`累计电费 ÷ 累计电量`），
    单一价直接取 `rate`（与「预估电费」同一口径）。
  - 含义：**以当前实时功率为基准外推全程**，等价于「假设开机以来一直维持当前负载」。
    与「累计电量/电费」不同——后者只统计本程序监测时段，不含监测开始前的时间。
- **三处展示**：
  1. 主界面读数卡片新增「开机以来」独占整行：左「开机时长」（如 `2天3时15分`）、
     右「开机至今电费（估）」（如 `¥18.62`，朱红高亮）。
  2. 顶部标题后的运行行：`运行 X天X时X分 · 开机电费 ¥X.XX · 日期`。
  3. 右侧硬件信息侧栏首行：`开机以来 X · 开机电费 ¥X.XX`。
- 新增辅助方法：`_fmt_dur()`（秒→中文时长）、`_blended_rate()`（有效电价）、
  `_boot_info()`（返回 `(时长文本, 电费, 秒数)`，读不到开机时间返回 None 优雅降级）。

### 验证
- 语法/导入检查通过；新增 `_smoke_v1847.py` 校验 `boot_dur/boot_cost` 控件存在且随
  `_sys_dyn["boot"]` 刷新；`tests/` 22 项、`_smoke_v1846.py` 布局冒烟通过。
- 桌面 EXE 实启动标题 `PC 电脑用电电费计算器 v18.47`。
...
各卡片有不同大小：硬件监测 720×260、功耗结构 500×570，其余卡片自动缩小；
并且各卡片可以拖动调整大小。本版按此重做。
...
VSplit（主）
├─ HSplit（上部）
│   ├─ VSplit（左）: 硬件监测 720×260 / 硬件信息（自动缩小）
│   └─ 功耗结构 500×570
└─ 管理功耗明细（自动缩小，底部通栏）
```

三个 `QSplitter` 嵌套，拖任意分隔条即可改变相邻卡片大小。
`setChildrenCollapsible(False)` 防止卡片被整个拖没后拉不回来；
分隔条加宽到 6px 并做 hover 变蓝，给出「可拖动」的视觉暗示。

### 两个实测踩到的坑

1. **`setSizes` 少减分隔条宽度**：`setSizes` 给的是分配额度，分隔条自身还要吃掉
   `handleWidth`。漏减这 6px 会让指定尺寸整体缩水（功耗结构实测 500 → 497）。
2. **「非内容区」高度不能硬编码**：窗口高 = 内容区 + 标题栏/控制条/边距/间距，
   后者与字体、DPI 相关。按估算值 158 写死后，实测 964 高的窗口里它只占 85px，
   功耗结构被挤成 643（预期 570）。改为首帧后**实测 `window.height() - split.height()`**
   再反推所需窗口高度，测得窗口 1242×891，两张指定卡片精确命中。

### 改动明细

- 删除 v18.44 的 `_PanelWindow` / `_PANEL_SPEC` / `_make_panels` / `_tile_panels` /
  `_toggle_panel` / `_show_all_panels` 及控制条面板开关行、托盘入口，四个卡片回到主窗口内
- 新增 `_build_main_split` / `_init_split_sizes` / `_reset_split_sizes` /
  `_restore_split_sizes` / `_main_window_size`
- 新增常量 `UI_CARD_LIVE=(720,260)`、`UI_CARD_BREAKDOWN=(500,570)`、
  `UI_SPLIT_HANDLE=6`、`UI_DETAIL_H=230`
- 会话持久化字段 `frames`（面板几何）改为 `split_state`（分隔条状态，base64），
  下次启动还原用户拖动出的卡片大小；还原失败自动回退默认尺寸
- 保留 v18.44 的界面紧凑化（页面/卡片边距、圆角、按钮内边距收紧）

### 验证

- `tests/` 22 项全过
- 离屏冒烟：可见顶层窗口 **1 个**（确认没有再被拆开）；硬件监测 **720×260**、
  功耗结构 **500×570** 精确命中；三个分隔条 QTest 鼠标拖动均生效
  （500→420 / 230→150 / 347→287）；分隔条状态存取还原正常

---

## v18.44 — 四个面板升级为独立顶层窗口（可单独最小化到任务栏），界面整体紧凑化

用户反馈：功耗结构 / 硬件信息 / 管理功耗明细 原来只是主窗口内的卡片，既不能单独拖动，
也没有任务栏条目、无法单独最小化（点最小化只会缩走整个主窗口）。本版把这四个单元
连同「硬件监测」一起提升为真正的独立顶层窗口。

### 关键修正：为什么之前任务栏里只有 main window

原先 four panels 以 `Qt.Tool`（工具窗口）创建：`Tool` 在 Windows 上**既不进任务栏、
也没有最小化按钮**，所以表现为「最小化=整个主窗口消失、面板变成孤儿窗口找不到」。
更正为 `Qt.WindowType.Window`（去掉 `skipTaskbar`）——每个面板由此获得
任务栏条目 + 最小化/最大化/关闭三键。

### 改动明细

- 新增 `_PanelWindow`：承载单个卡片的独立顶层窗口。
  - `Qt.WindowType.Window`：独立任务栏条目，可单独最小化 / 拖动 / 拖边框缩放。
  - `WA_DeleteOnClose = False` + `closeEvent` 改为 hide：真关闭会连同
    `self.table` / `self.wall_big` / `self.chart_view` 等 C++ 子树一起销毁，
    下一次采样刷新就是悬空访问崩溃（这是 QWidget 复用场景最容易踩的雷）。
- 四个面板与预设尺寸（精确按用户指定）：

  | 面板 | 尺寸 | 内容来源 |
  |---|---|---|
  | 硬件监测 | 720 × 260 | `_live_card()`：插座功耗 / 累计电量 / 倒计时 / 负载 / 预估电费 |
  | 硬件信息 | 500 × 570 | `_sysinfo_panel()`：CPU/主板/GPU/内存/磁盘/风扇明细 |
  | 功耗结构 | 500 × 570 | `_breakdown_card()`：四列构成表（部件/使用率/功耗/温度·转速） |
  | 管理功耗明细 | 1160 × 710 | `_chart_card()`：近 60 分钟功耗曲线 |

- 面板 reparent 后，`MainWindow` 里原有的成员引用（`self.wall_big`、`self.table`、
  `self.chart_view`、`self.sysinfo_view` …）**保持不变**，采样刷新照旧写入，
  Qt 会把重绘投递到新的父窗口 —— 上层业务逻辑零改动。
- `_tile_panels()`：首次显示时把四个面板从左到右错开摆放（宽度超出屏幕可用宽自动换行），
  避免层层叠在左上角。
- 控制条新增第二行「面板窗口：」——四个 toggle 按钮 + 「全部显示」，
  面板被隐藏时按钮勾选状态自动同步（`_on_panel_hidden`）。
- 托盘菜单新增「显示全部面板窗口」。
- 主窗口从 1080×920 收缩到 1040×190：四个单元移出后只剩标题行 + 控制条。
- **界面紧凑化**：页面外框 18/16 → 8/7，卡片 margin 16/14 → 8/6，卡片内边距 14/12 → 6/5，
  元素间距 8 → 3，硬件卡列间距 22 → 12，读数网格 16/10 → 10/6；小tent努力性的同时保持可读。
- `local_temp_monitor` 彻底不再被引用（v18.43 已切回 LHM，本版无残留）。

### 遗留说明

- App 退出时不会主动结束内置 LibreHardwareMonitor 后台进程（约 95MB 常驻，避免重启重拉起
  的 7 秒冷启动）。需要一并回收时可另行处理。
- 面板位置本次刻意**不落盘**：每次启动重新按 `_tile_panels()` 错开摆放，
  避免历史坐标把窗口丢到屏幕外。
